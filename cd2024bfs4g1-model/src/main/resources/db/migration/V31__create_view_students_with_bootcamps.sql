...
SELECT
st.*,uu.*,ss.status ,vres.employment_status,concat(concat('|',string_agg(sb.bootcamp_id::text,'|')),'|') bootcamps_id
FROM
students st
...
group by st.id,ss.status,vres.employment_status,uu.usr_id