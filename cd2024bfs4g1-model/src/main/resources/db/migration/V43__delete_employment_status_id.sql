...
--v_students_with_bootcamps
DROP VIEW IF EXISTS v_students_with_bootcamps;

CREATE OR REPLACE VIEW public.v_students_with_bootcamps AS
SELECT
    st.id,
    st.name,
    st.surname1,
...
FROM
    students st
LEFT JOIN
    usr_user uu ON st.user_id = uu.usr_id
LEFT JOIN
    student_status ss ON st.student_status_id = ss.id
LEFT JOIN
    v_recent_employment_status vres ON st.id = vres.student_id
LEFT JOIN
    student_bootcamp sb ON sb.student_id = st.id
LEFT JOIN
    tutors t ON t.id = st.tutor
LEFT JOIN (
    SELECT
        count(*) AS validos,
        sb_1.student_id
    FROM
        student_bootcamp sb_1
    JOIN
        bootcamps b ON b.id = sb_1.bootcamp_id
    WHERE
        b.end_date >= CURRENT_DATE
    GROUP BY
        sb_1.student_id
) bot_activ ON st.id = bot_activ.student_id
GROUP BY
    t.name, t.surname1, t.surname2, st.id, ss.status,
    vres.employment_status, vres.v_employment_status_id,
    uu.usr_id, bot_activ.student_id, bot_activ.validos
ORDER BY
    (st.name COLLATE "es-ES-x-icu");
